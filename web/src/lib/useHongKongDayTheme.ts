...
export function useHongKongDayTheme(): HongKongDayTheme {
  ...
  useEffect(() => {
    ...
    root.style.setProperty("--page-surface", "#131313");
    root.style.setProperty("--page-surface-hover", "#1a1a1a");
  ...
  const onScrubWheel = (event: ReactWheelEvent<HTMLElement>) => {
    ...
    bumpMinuteFromWheel(event.deltaY, event.shiftKey);
  };

  const onClockWheel = useCallback(
    (event: WheelEvent) => {
      event.preventDefault();
      event.stopPropagation();
      bumpMinuteFromWheel(event.deltaY, event.shiftKey);
  ...
  return {
    ...
    localClockHref: `https://www.timeanddate.com/worldclock/results.html?query=${encodeURIComponent(location.city)}`,
    isScrubbing,
    onScrubWheel,
    onClockWheel,
    resetScrub,
