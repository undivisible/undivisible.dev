...
fn launch(app_index: usize) void {
    ...
    if (std.mem.eql(u8, an, "web"))
        handOver("netsurf-fb -f linux -w 1440 -h 900 file:///usr/share/alpenglowed/web/index.html");
    if (std.mem.eql(u8, an, "links"))
        handOver("links -g -driver fb file:///usr/share/alpenglowed/web/index.html");
    if (a.builtin) {
        var name_z: [64]u8 = std.mem.zeroes([64]u8);
        @memcpy(name_z[0..an.len], an);
        spawn("/usr/bin/alpenpanel", @ptrCast(&name_z));
    } else {
        var path: [288]u8 = std.mem.zeroes([288]u8);
...
pub fn main() void {
    ...
    spawn("/usr/bin/alpenwall", null);
    spawn("/usr/bin/alpenclock", null);
    spawn("/usr/bin/alpenmachine", null);
    spawn("/usr/bin/alpenname", null);
